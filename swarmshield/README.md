...
- [x] Module 3: LangChain middleware + LangGraph gate (`swarmshield/integrations/`, see [docs/INTEGRATIONS.md](../docs/INTEGRATIONS.md))
...
uvicorn swarmshield.gateway:app --port 8100   # 8000 is the platform API in docker-compose
```
Try it:
```bash
curl -s -X POST localhost:8100/a2a/transfer -H 'content-type: application/json' -d '{
...
# -> HTTP 403
```
Verdicts: 200 allow/flag, 403 blocked, 429 circuit breaker tripped. Live events: `ws://localhost:8100/ws/telemetry`.
