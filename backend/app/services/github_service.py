...
async def write_remediation_branch(
    *,
    vulnerability_id: uuid.UUID,
    patch_id: uuid.UUID,
    patch_summary: str,
    ...
    """Creates a review branch with the remediation artifact but does not
    open a pull request. Used for private repos where the operator has
    granted write access but wants branch-only output."""
    if not settings.github_configured():
        return {
            "configured": False,
            "status": "not_configured",
            ...
            "pr_number": None,
            "pr_url": None,
            "error": "GitHub integration is not configured (GITHUB_TOKEN and/or GITHUB_REPO unset).",
        }
    ...
    file_path = f"swarmshield-remediations/{vulnerability_id}.md"
    file_content = (
        f"# SwarmShield remediation branch\n\n"
        f"- Vulnerability: `{vulnerability_id}`\n"
        f"- Patch: `{patch_id}`\n"
        f"- Type: `{patch_type}`\n\n"
        f"## Summary\n{patch_summary}\n\n"
    ...
    async with httpx.AsyncClient(base_url=settings.GITHUB_API_URL, timeout=20.0) as client:
        try:
            base_sha = await _get_base_branch_sha(client, repo, base_branch)
            await _create_branch(client, repo, branch_name, base_sha)
            await _commit_patch_file(
                client,
                repo,
                branch_name,
                file_path,
                file_content,
                f"SwarmShield: branch remediation for {str(vulnerability_id)[:8]}",
            )
            return {
                "configured": True,
                "status": "created",
